...
const log = (event, extra = {}) =>
  console.log(JSON.stringify({ event, service: 'account-service', ts: new Date().toISOString(), ...extra }));
...
app.get('/health', (req, res) => {
  res.status(200).json({ status: 'ok', service: 'account-service', ts: new Date().toISOString() });
});
